...
bool CartesianPosVelController::init(hardware_interface::RobotHW* robot_hw,
    ...
    dynamic_server_posvel_param_ = std::make_unique<
        dynamic_reconfigure::Server<za_controllers::posvel_paramConfig>>(
            dynamic_reconfigure_posvel_param_node_);
    
    dynamic_server_posvel_param_->setCallback(
        boost::bind(&CartesianPosVelController::posvelParamCallback, this, _1, _2));
...
void CartesianPosVelController::update(const ros::Time& /*time*/, const ros::Duration& period) {
    ...
    Eigen::Vector3d orient_error = Ko_ * orient_errorq.vec();
...
void CartesianPosVelController::posvelParamCallback(za_controllers::posvel_paramConfig& config,
                                                    uint32_t /*level*/) {
